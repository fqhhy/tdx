use std::{
    io::{Read, Result, Write},
    net::TcpStream,
    time::Duration,
};

use log::trace;
use tdx::parse::parse_kline;

pub const PACK1: &[u8] = &[
    0x0c, 0x02, 0x18, 0x93, 0x00, 0x01, 0x03, 0x00, 0x03, 0x00, 0x0d, 0x00, 0x01,
];
pub const PACK2: &[u8] = &[
    0x0c, 0x02, 0x18, 0x94, 0x00, 0x01, 0x03, 0x00, 0x03, 0x00, 0x0d, 0x00, 0x02,
];
pub const PACK3: &[u8] = &[
    0x0c, 0x03, 0x18, 0x99, 0x00, 0x01, 0x20, 0x00, 0x20, 0x00, 0xdb, 0x0f, 0xd5, 0xd0, 0xc9, 0xcc,
    0xd6, 0xa4, 0xa8, 0xaf, 0x00, 0x00, 0x00, 0x8f, 0xc2, 0x25, 0x40, 0x13, 0x00, 0x00, 0xd5, 0x00,
    0xc9, 0xcc, 0xbd, 0xf0, 0xd7, 0xea, 0x00, 0x00, 0x00, 0x02,
];

const KLINE_PACK: &[u8] = &[
    0x0c, 0x01, 0x08, 0x64, 0x01, 0x01, 0x1c, 0x00, 0x1c, 0x00, 0x2d, 0x05, 0x00, 0x00, 0x30, 0x30,
    0x30, 0x30, 0x30, 0x31, 0x09, 0x00, 0x01, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
];

// tick/img 请求模板，offset 在 [12..16]，count 在 [16..18]
const IMG_PACK: &[u8] = &[
    0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x36, 0x1, 0x36, 0x1, 0xb9, 0x6, // 超级盘口
    0x00, 0x00, 0x00, 0x00, // offset [12..16]
    0x30, 0x75, // count
    0x00, 0x00, // ??
    0x68, 0x69, 0x73, 0x68, 0x66, 0x2f, //  hishf/
    0x64, 0x61, 0x74, 0x65, 0x2f, // date/
    0x32, 0x30, 0x32, 0x36, 0x30, 0x33, 0x31, 0x37, 0x2f, // 20260313/
    0x73, 0x7a, 0x30, 0x30, 0x30, 0x30, 0x30, 0x31, // sz000001
    0x2e, 0x69, 0x6d, 0x67, // .img
    ,
];

pub fn send_recv(tcp: &mut TcpStream, send: &[u8]) -> Result<Vec<u8>> {
    tcp.write_all(send)?;
    let mut head = [0u8; 16];
    let _head_size = tcp.read(&mut head)?;
    let deflate_size = u16::from_le_bytes([head[12], head[13]]); // 响应信息中的待解压长度
    let mut buf = vec![0; deflate_size as usize];
    tcp.read_exact(&mut buf)?;
    let inflate_size = u16::from_le_bytes([head[14], head[15]]); // 响应信息中的解压后长度
    if deflate_size != inflate_size {
        let mut decompressed = vec![0u8; inflate_size as usize];
        let (result, rc) =
            zlib_rs::decompress_slice(&mut decompressed, &buf, zlib_rs::InflateConfig::default());
        assert_eq!(rc, zlib_rs::ReturnCode::Ok);
        buf = result.to_vec();
        trace!("解压后数据：\n{:?}\n", buf);
    } else {
        trace!("无需解压: \n{:?}\n", buf);
    };

    Ok(buf)
}

fn test_super_tick(socket:&mut TcpStream) {
    let mut all = Vec::new();
    let step: u32 = 0x7530; // 每次步进 65536 字节
    for i in 0..16u32 {
        let offset: u32 = i * step;
        let mut pkt = IMG_PACK.to_vec();
        pkt[12..16].copy_from_slice(&offset.to_le_bytes());

        let res = send_recv(socket, &pkt).unwrap();
        let preview = &res[..res.len().min(50)];
        println!(
            "offset=({:>8}) len={:>6} 前50字节: {:?}",
            offset,
            res.len(),
            preview
        );
        all.extend_from_slice(&res[4..]);
        if res.len() - 4 < step as usize {
            println!("已到文件末尾，停止请求");
            break;
        }
    }
    // all 保存到文件
    std::fs::write("sz000001-20260317.img", &all);
}

fn main() -> std::io::Result<()> {
    // 初始化日志系统
    env_logger::init();
    match TcpStream::connect(("129.211.70.79", 7709)) {
        Ok(mut socket) => {
            socket
                .set_read_timeout(Some(Duration::from_secs(5)))
                .unwrap();
            socket
                .set_write_timeout(Some(Duration::from_secs(5)))
                .unwrap();

            send_recv(&mut socket, &PACK1)?;
            send_recv(&mut socket, &PACK2)?;
            send_recv(&mut socket, &PACK3)?;

            println!("Connected to server");

            let mut arr = [0; KLINE_PACK.len()];
            arr.copy_from_slice(KLINE_PACK);

            let market: u16 = 0;
            let code: &str = "000001";
            let category: u16 = 9;
            let start: u16 = 0;
            let count: u16 = 3;
            arr[12..14].copy_from_slice(&market.to_le_bytes());
            arr[14..20].copy_from_slice(code.as_bytes());
            arr[20..22].copy_from_slice(&category.to_le_bytes());
            arr[24..26].copy_from_slice(&start.to_le_bytes());
            arr[26..28].copy_from_slice(&count.to_le_bytes());

            let res = send_recv(&mut socket, &arr)?;
            let klines = parse_kline(res, category);
            println!("{:?}", klines);

            // 发送自定义数据包
            let custom_pack: &[u8] = &[
                0x0c, 0x36, 0x00, 0x29, 0x00, 0x01, 0x0f, 0x00, 0x0f, 0x00, 0x47, 0x05, 0x01, 0x00,
                0x00, 0x31, 0x35, 0x39, 0x31, 0x32, 0x37, 0x00, 0x00, 0x00, 0x00,
            ];
            let custom_res = send_recv(&mut socket, custom_pack)?;
            println!("返回数据 ({} 字节): {:?}", custom_res.len(), custom_res);
            println!("ASCII: {}", String::from_utf8_lossy(&custom_res));

        }
        Err(e) => {
            eprintln!("连接失败: {}", e);
        }
    };
    Ok(())
}
